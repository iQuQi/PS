# 단어배열과 쿼리 배열이 주어진다
# 쿼리는 fro?? 처럼 소문자와 ?로 이루어짐
# 각 쿼리별로 매치된 단어가 몇개인지 출력하기
...
def solution(words, queries):
    ...
    for query in queries:  
        ...
        count = 0
        if query[-1] == '?': 
            left = bisect_left(array[len(query)],left_query)
            right = bisect_right(array[len(query)],right_query)
    
        elif query[0] == '?':
            left = bisect_left(reverse_array[len(query)],left_query[::-1])
            right = bisect_right(reverse_array[len(query)],right_query[::-1])
